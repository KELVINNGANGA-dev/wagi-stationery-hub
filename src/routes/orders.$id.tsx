...
import { ORDER_STATUS_FLOW, ORDER_STATUS_LABELS, PAYMENT_STATUS_LABELS, STORE } from "@/lib/store-config";
...
export const Route = createFileRoute("/orders/$id")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Follow your WAGI order from packing to delivery." },
      { property: "og:title", content: "Order Details | WAGI - STATIONARIES" },
      { property: "og:description", content: "Live status for your stationery order." },
  ...
  if (!order) {
    return (
      <div className="container-page py-20 text-center">
        ...
        <Link to="/orders" className="mt-3 inline-block text-sm text-primary hover:underline">
          Back to my orders
        </Link>
      </div>
    );
      ...
      <div className="mt-8 grid gap-6 lg:grid-cols-[1fr_340px]">
        <section className="rounded-2xl border bg-card p-5 shadow-card">
          ...
          <div className="mt-4 space-y-3">
            {items.map((it) => (
              <div key={it.id} className="flex items-center justify-between gap-3 text-sm">
                <span className="line-clamp-2 flex-1">{it.product_name}</span>
                <span className="text-muted-foreground">×{it.quantity}</span>
                <span className="font-semibold">{formatPrice(it.total_price)}</span>
              </div>
            ))}
          </div>
          ...
          <div className="space-y-1.5 text-sm">
            ...
            <Row
              label="Delivery"
              value={Number(order.delivery_fee) === 0 ? "Free" : formatPrice(order.delivery_fee)}
            />
            {Number(order.tax) > 0 && <Row label="Tax" value={formatPrice(order.tax)} />}
        ...
        <aside className="space-y-4">
          <div className="rounded-2xl border bg-card p-5 text-sm shadow-card">
            ...
            <p className="text-muted-foreground">
              {[order.house_number, order.building, order.street, order.estate, order.town, order.county]
                .filter(Boolean)
                .join(", ")}
            </p>
            {order.landmark && (
          ...
          <div className="rounded-2xl border bg-card p-5 text-sm shadow-card">
            ...
            <p className="mt-2 text-muted-foreground">
              Call <a href={`tel:${STORE.phoneIntl}`} className="text-primary">{STORE.phone}</a> or
              email{" "}
              <a href={`mailto:${STORE.email}`} className="break-all text-primary">
                {STORE.email}
              </a>
              .
            </p>
